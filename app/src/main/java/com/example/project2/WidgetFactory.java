...
import com.example.project2.utils.IconCache;
...
public class WidgetFactory implements RemoteViewsService.RemoteViewsFactory {
    ...
    private void loadData() {
        ...
        this.category = prefs.getString(KEY_CATEGORY + appWidgetId, "All");
        List<AppInfo> loadedApps = AppManager.loadAppsSync(context, category, false);
        for (AppInfo app : loadedApps) {
            Bitmap cachedIcon = iconCache.getIcon(app.getPackageName());
            if (cachedIcon != null) {
    ...
    private void loadIconAsync(AppInfo app, RemoteViews views, int position) {
        new Thread(() -> {
            Bitmap bitmap = iconCache.loadIcon(app.getPackageName());
            if (bitmap != null) {
    ...
    public RemoteViews getLoadingView() {
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.item_widget_app);
        views.setTextViewText(R.id.widget_app_name, "Загрузка...");
